...
#include "mlir/Polygeist/Dialect/PolygeistOps.h"
#include "mlir/Polygeist/Transforms/Passes.h"
#include "mlir/Polygeist/Utils/BarrierUtils.h"
#include "mlir/Polygeist/Utils/Utils.h"
...
static LogicalResult distributeAroundBarrier(T op, polygeist::BarrierOp barrier,
...
static polygeist::BarrierOp getFirstBarrier(Block *block) {
  ...
  return cast<polygeist::BarrierOp>(&*it);
}

template <typename T, bool UseMinCut>
static LogicalResult distributeAroundFirstBarrier(T op, T &preLoop, T &postLoop,
                                                  PatternRewriter &rewriter) {
  polygeist::BarrierOp barrier = getFirstBarrier(op.getBody());
  if (!barrier)
    return failure();
...
template <typename T, bool UseMinCut>
static LogicalResult
distributeAfterWrap(Operation *pop, polygeist::BarrierOp barrier,
...
static polygeist::BarrierOp getBarrierInBlock(Block *block) {
  for (Operation &op : *block) {
    if (auto barrier = dyn_cast<polygeist::BarrierOp>(&op))
      return barrier;
  }
  return nullptr;
}
...
struct DistributeIfAroundBarrier : public OpRewritePattern<IfOpType> {
  ...
  LogicalResult matchAndRewrite(IfOpType ifOp,
    ...
    polygeist::BarrierOp thenBarrier = nullptr;
    polygeist::BarrierOp elseBarrier = nullptr;
    if (thenBlock)
    ...
    auto handleCase = [&ifPre, &rewriter, &ifOp](polygeist::BarrierOp barrier,
...
struct Reg2MemFor : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T op,
    ...
    while (IP != &IP->getBlock()->front()) {
      if (isa<polygeist::BarrierOp>(IP->getPrevNode())) {
        IP = IP->getPrevNode();
      }
      break;
    }
...
std::unique_ptr<Pass>
mlir::polygeist::createDistributeBarriersPass(DistributeBarriersOptions str) {
  return std::make_unique<DistributeBarriers>(str);
}
std::unique_ptr<Pass> mlir::polygeist::createDistributeBarriersPass() {
